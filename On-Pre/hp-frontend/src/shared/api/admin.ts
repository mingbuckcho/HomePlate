...
import { getGamesByTeam } from "@/shared/api/info";
import type { GameResponse, PageOutboxHistory } from "@/shared/api/types";
...
/**
 * 관리자 경기 목록: /info/games/top5는 5개만 주므로,
 * 팀별 API를 모두 호출해서 합친 뒤 gameId 기준 중복 제거·일시순 정렬.
 */
export async function getAdminGameList(): Promise<GameResponse[]> {
  const teamIds = ADMIN_TEAM_OPTIONS.map((o) => o.value);
  const results = await Promise.all(
    teamIds.map((teamId) => getGamesByTeam(teamId)),
  );
  const byId = new Map<number, GameResponse>();
  for (const list of results) {
    for (const g of list) byId.set(g.gameId, g);
  }
