...
public enum ErrorCode {
    ...
    SEAT_LOCK_EXPIRED_OR_NOT_MINE(HttpStatus.CONFLICT, "좌석 선점 시간이 만료되었거나, 본인이 선점한 좌석이 아닙니다."),

    // 400 BAD_REQUEST: 잘못된 요청
    REFRESH_TOKEN_MISSING(HttpStatus.BAD_REQUEST, "Refresh Token이 쿠키에 존재하지 않습니다."),
    INVALID_SEAT_REQUEST(HttpStatus.BAD_REQUEST, "요청한 좌석 정보가 유효하지 않습니다."),
    USER_NOT_MATCH(HttpStatus.BAD_REQUEST, "본인의 주문만 결제할 수 있습니다."),
    INVALID_ORDER_STATUS(HttpStatus.BAD_REQUEST, "유효하지 않은 주문 상태입니다."),
    MAX_TICKET_LIMIT_EXCEEDED(HttpStatus.BAD_REQUEST, "1인당 경기별 최대 4매까지만 예매 가능합니다."),

    // 401 UNAUTHORIZED
    PASSWORD_NOT_MATCH(HttpStatus.UNAUTHORIZED, "비밀번호가 일치하지 않습니다."),
    INVALID_TOKEN(HttpStatus.UNAUTHORIZED, "유효하지 않은 토큰입니다."),
    REFRESH_TOKEN_NOT_FOUND(HttpStatus.UNAUTHORIZED, "로그아웃된 사용자이거나 토큰이 만료되었습니다."),
    LOGIN_REQUIRED(HttpStatus.UNAUTHORIZED, "로그인이 필요한 서비스입니다."),
    QUEUE_TOKEN_NOT_FOUND(HttpStatus.UNAUTHORIZED, "대기열 정보가 존재하지 않습니다. 줄을 서주세요."),

    // 403 FORBIDDEN
    ACCESS_DENIED_NOT_ACTIVE(HttpStatus.FORBIDDEN, "아직 입장 순서가 아닙니다. 대기해주세요."),
