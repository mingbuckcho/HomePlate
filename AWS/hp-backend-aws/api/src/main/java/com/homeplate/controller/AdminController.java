...
public class AdminController {
    ...
    @GetMapping("/games")
    @Operation(summary = "관리자용 경기 목록 조회", description = "상태, 오픈시간 등 관리자 전용 데이터가 포함된 목록을 조회합니다.")
    public ResponseEntity<Page<GameResponse>> getAllGames(
            @PageableDefault(size = 10, sort = "gameStartAt", direction = Sort.Direction.ASC) Pageable pageable) {

        return ResponseEntity.ok(service.getAllGames(pageable));
    }
