...
# 잡 타입별 unhealthy 전환 threshold (연속 실패 횟수)
#
# 왜 타입별로 다르게 설정하는가:
#   잡마다 실행 주기와 데이터 중요도가 다르다.
#   팀 순위/경기 일정은 하루 한 번 수집이라 2번 실패하면
#   이미 수 시간치 데이터가 누락된 상태다.
#   뉴스/굿즈는 자주 실행되므로 일시적 실패를 더 허용한다.
#
# 미등록 타입은 DEFAULT_UNHEALTHY_THRESHOLD를 사용한다.
DEFAULT_UNHEALTHY_THRESHOLD = 3
UNHEALTHY_THRESHOLD: dict[str, int] = {
    "CRAWL_KBO_TEAM_RANK": 2,
    "CRAWL_KBO_GAMES": 2,
    "CRAWL_NAVER_NEWS": 4,
    "CRAWL_KBOMARKET_GOODS": 3,
    "DISCOVER_KBOMARKET_GOODS": 3,
}


def _get_threshold(job_type: str) -> int:
    return UNHEALTHY_THRESHOLD.get(job_type, DEFAULT_UNHEALTHY_THRESHOLD)


def _transition_unhealthy(db, job_type: str, err_msg: str) -> None:
    """
    job_health가 healthy(또는 없음) → unhealthy로 전환할 때만 Slack 알림을 보낸다.
    이미 unhealthy 상태면 아무것도 하지 않는다.

    왜 상태 전환에만 알림을 보내는가:
        장애가 지속되는 동안 매 실패마다 알림을 보내면
        운영자가 알림을 무시하게 된다 (Alert Fatigue).
        "처음 장애를 감지했을 때 1번"만 알리고
        복구됐을 때 1번 알리는 것이 실무에서 표준적인 방식이다.
    """
    prev = db.job_health.find_one({"type": job_type})
    if prev and prev.get("status") == "unhealthy":
        return  # 이미 알림을 보낸 상태 → 중복 알림 방지

    db.job_health.update_one(
        {"type": job_type},
        {"$set": {"status": "unhealthy", "since": utc_now()}},
        upsert=True,
    )
    send_slack(
        f"[장애] `{job_type}` 연속 {_get_threshold(job_type)}회 실패\n"
        f"마지막 에러: {(err_msg or '')[:200]}"
    )


def _transition_healthy(db, job_type: str) -> None:
    """
    job_health가 unhealthy → healthy로 전환할 때만 Slack 복구 알림을 보낸다.
    이미 healthy 상태면 아무것도 하지 않는다.
    """
    prev = db.job_health.find_one({"type": job_type})
    if not prev or prev.get("status") == "healthy":
        return  # 정상 상태였으면 복구 알림 불필요

    db.job_health.update_one(
        {"type": job_type},
        {"$set": {"status": "healthy", "since": utc_now()}},
    )
    send_slack(f"[복구] `{job_type}` 정상화됨")
...
def complete_job(job_id) -> None:
    ...
    왜 attempts를 0으로 리셋하는가:
        threshold는 "연속 실패 횟수"를 기준으로 한다.
        성공 후 다시 실패하면 새로운 장애로 판단해야 하므로
        성공 시 attempts를 초기화한다.
    """
    db = get_db()
    now = utc_now()
    ...
    if job:
        _transition_healthy(db, job.get("type", ""))
...
def fail_job(job_id, err_msg: str) -> None:
    ...
    if job:
        ...
        if attempts >= _get_threshold(job_type):
            _transition_unhealthy(db, job_type, err_msg)
