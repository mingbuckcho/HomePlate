...
def get_s3():
    """
    boto3 S3 클라이언트 인스턴스를 생성해서 반환한다.

    자격증명은 boto3 기본 체인에 위임한다:
      - 운영(EKS): IRSA가 자동으로 임시 자격증명을 주입
      - 로컬: ~/.aws/credentials 또는 AWS_PROFILE 환경변수
    """
...
def upload_thumbnail(
    ...
    """
    이미지 URL에서 이미지를 다운로드해서 S3에 업로드한다.

    Args:
        image_url: 다운로드할 이미지의 원본 URL
        prefix:    S3 저장 경로 구분자 (예: "naver_news", "kbomarket_goods")
        source:    소스 키 - 헤더 선택에 사용 (예: "naver", "kbomarket")

    Returns:
        S3 경로 (예: "naver_news/abc123.jpg")
    ...
    try:
        ...
        return object_name  # DB에 저장할 S3 경로

    except Exception as e:
        # 업로드 실패는 수집을 멈추지 않는다
        # 원본 URL을 반환해서 데이터 손실을 막는다
        print(f"[s3] 업로드 실패, 원본 URL 사용: {image_url} / {e}")
        return image_url
